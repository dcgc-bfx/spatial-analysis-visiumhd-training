...
p2 = SpatialFeaturePlot(sc, features="pMito_RNA.008um") 
p1 | p2
```
...
filtered_bins_mito = WhichCells(sc, expression=pMito_RNA.008um>40)
...
sc = PercentageFeatureSet(sc, pattern="^Hb[ab]-", col.name="pHemoglobin.008um") 

p1 = VlnPlot(sc, features="pHemoglobin.008um", pt.size=0, layer="counts") + NoLegend()
p2 = SpatialFeaturePlot(sc, features="pHemoglobin.008um") 
p1 | p2
```
...
sc = subset(sc, pMito_RNA.008um > 40, invert=TRUE) %>% suppressWarnings()
sc[["RNA.008um"]]
```
...
p1 = DimPlot(sc, reduction="pca.sketch", dims=c(1, 2)) + NoLegend()
p2 = DimPlot(sc, reduction="pca.sketch", dims=c(2, 3)) + NoLegend()
p3 = DimPlot(sc, reduction="pca.sketch", dims=c(1, 3)) + NoLegend()
...
# Suggestion: use a cluster tree
# sc.ct = BuildClusterTree(sc)
# PlotClusterTree(sc)
```
...
sc = RunUMAP(sc, reduction="pca.sketch", reduction.name="umap.sketch.test", dims=1:5, 
             n.neighbors=100)
...
p2 = SpatialDimPlot(sc, pt.size.factor=5) + NoLegend() + scale_fill_manual(values=cal_pal50)
p1 | p2
```
...
A **violin plot** allows us to visualize the distribution of gene expression across clusters. For example, we can plot the expression of genes Ptgds, Acta2 and Apoe:

```{r}
#| label: plotting_1

genes = c("Ptgds", "Acta2", "Apoe")
VlnPlot(sc, features=genes, pt.size=0, ncol=1) + NoLegend()
```

We can also visualize the gene expression directly on top of the **UMAP plot**. This will show the expression of genes such as Ptgds, Acta2 and Apoe across all cells in the low-dimensional UMAP space:

```{r}
#| label: plotting_2

FeaturePlot(sc, features=genes)
```
...
SpatialFeaturePlot(sc, features=genes, image.alpha=0.1)
```
...
p1 = SpatialDimPlot(sc, group.by="seurat_clusters.008um") + NoLegend() + ggtitle("Seurat") + scale_fill_manual(values=cal_pal50)
p2 = SpatialDimPlot(sc, group.by="banksy_clusters.008um") + NoLegend() + ggtitle("BANKSY") + scale_fill_manual(values=cal_pal50)
p3 = SpatialDimPlot(sc, group.by="banksy_domains.008um") + NoLegend() + ggtitle("BANKSY") + scale_fill_manual(values=cal_pal50)
(p1 | p2) / p3
p2 | p3
```
...
# Make diet - get rid of old graphs, neighbors, ...
cortex = DietSeurat(cortex)

# Not entirely clean
cortex[["sketch"]] = NULL
cortex@tools$TransferSketchLabels = NULL
...
cortex = SketchData(
  ...
  over.write = TRUE,
  features = VariableFeatures(cortex)
)
...
# Neighbors and UMAP
cortex = FindNeighbors(cortex, reduction="pca.sketch", dims=1:50)
cortex = RunUMAP(cortex, reduction="pca.sketch", reduction.name="umap.sketch", return.model=TRUE, dims=1:50, verbose=TRUE)
```
...
# Project results back to full cortex dataset
...
SpatialDimPlot(sc, cells.highlight=layered_excitatory_bins, facet.highlight=TRUE, cols.highlight=c("#FFFF00", "grey50"), combine=TRUE) + NoLegend()
```

Note that the RCTD approach identified only a subset of cells and they are difficult to see in the plot. Therefore, we try to use these predictions to annotate clusters.
